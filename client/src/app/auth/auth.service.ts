...
export class AuthService {
  ...
  signup(email: string, password: string, isTrader: boolean) {
    ...
    this.http
      .post<{ message: string; result: any; signup: boolean }>(
  ...
  getIsTrader(): any {
    this.http
      .get<{ isTrader: boolean }>("http://localhost:9000/users/isTrader")
      .subscribe(response => {
  ...
  getIsAdmin() {
    return this.http.get<{ isAdmin: boolean }>(
      "http://localhost:9000/users/isAdmin"
    );
  }
